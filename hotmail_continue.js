...
var PROXY_PORT="154.6.244.191:3128"
var EMAIL_ID="sarahrandallmxt3227@hotmail.com"
var EMAIL_PWD="u2WW9io1AU"
...
for(i=0;i<5; i++){ 
    ...
    iimPlay(load);
    EMAIL_ID = iimGetLastExtract(1);
    EMAIL_PWD = iimGetLastExtract(2);
    var proxyIP = iimGetLastExtract(3);
    var proxyPort = iimGetLastExtract(4);
    PROXY_PORT=proxyIP+":"+proxyPort;
    
    if(EMAIL_ID==null || EMAIL_ID.indexOf("@")===-1 || EMAIL_ID.indexOf(",")>=0){
    	continue;
    }
    //alert(EMAIL_ID+"<==>"+EMAIL_PWD+"<==>"+proxyIP+"<==>"+proxyPort+"<==**>"+EMAIL_ID.indexOf(","))
...
signout = renderJS;
signout += "TAG XPATH=\".//button[@aria-label='Account' and @title='Account']\""+"\n";
signout += "WAIT SECONDS=2.0 \n";
signout += "TAG XPATH=\".//div[text()='Sign out']/ancestor::button\""+"\n";
signout += "TAG XPATH=\".//button[@aria-label='Account manager']\""+"\n";
signout += "WAIT SECONDS=2.0 \n";
signout += "TAG XPATH=\".//a[@id='meControlSignoutLink']\""+"\n";
...
var myCategories = ["Green", "Red", "Yellow", "Orange", "Blue", "Purple"];
		...
		while(1) {
					...
					k=k+1;
					numberOfmarkAsNotJunk=getRndInteger(randomMinJunk,randomMaxJunk);
					numberOfInboxProcess=getRndInteger(randomMaxInbox,randomMinInbox);
					randomSleep=0;
						...
						for(var c=0; c<numberOfmarkAsNotJunk; c++) {
					    	...
					    	iimPlay(moveToInbox);
					    	moveToInbox="";
					    	//alert(c+" -- "+numberOfmarkAsNotJunk)
					    }
					    
					    clickInbox="";
					    clickInbox = renderJS;
					    clickInbox += "TAG POS=1 TYPE=SPAN ATTR=TXT:Inbox" + "\n";
						clickInbox += "WAIT SECONDS=3.0 " + "\n";

						iimPlay(clickInbox);

						filterUnread = renderJS;

						filterUnread += "TAG XPATH=\".//*[text()='Filter']\"" + "\n";
						filterUnread += "TAG POS=1 TYPE=BUTTON ATTR=NAME:Unread" + "\n";
						filterUnread += "WAIT SECONDS=6.0" + "\n";
						//iimPlay(filterUnread);
						filterUnread ="";
						...
						for(var c=0; c<numberOfInboxProcess; c++) {
						    ...
						    //processInbox1 += "TAG XPATH=\".//*[text()='Filter']\"" + "\n";
							//processInbox1 += "TAG POS=1 TYPE=BUTTON ATTR=NAME:Unread" + "\n";
							//processInbox1 += "WAIT SECONDS=8.0" + "\n";


						    //processInbox += "TAG XPATH=\".//*[text()='Filter']\"" + "\n";
					   ...
					   signout = renderJS;
					   signout += "TAG XPATH=\".//button[@aria-label='Account' and @title='Account']\""+"\n";
					   signout += "WAIT SECONDS="+randomDelayLoop+" \n";
					   signout += "TAG XPATH=\".//div[text()='Sign out']/ancestor::button\""+"\n";
					   signout += "TAG XPATH=\".//button[@aria-label='Account manager']\""+"\n";
					   signout += "WAIT SECONDS="+randomDelayLoop+" \n";
					   signout += "TAG XPATH=\".//a[@id='meControlSignoutLink']\""+"\n";
